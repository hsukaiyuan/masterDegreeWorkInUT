...
def analyze_sdof_response(acceleration, dt, k, c, m, j, folder_path):
    ...
    max_value = np.max(np.abs(u_df))
    max_index = np.argmax(np.abs(u_df))
    plt.axvline(x=max_index, color='gray', linestyle='--', label=f'Max Value: {max_value:.2f}')
...
def objective(trial, folder_path, low_bound_data_new, up_bound_data_new, decoder, vmax, vmin, reconTimeSeries):
    global j, best_max_displacement
    ...
    if max_displacement > best_max_displacement:
        best_max_displacement = max_displacement
        ground_motion_file_name = generate_filename(folder_path, f"ground_acceleration_{i+1}_{j}")
        visualize_seismic(ground_motion_acceleration, title=f"Optimized Seismic Acceleration_{i+1}_{j}", ylabel="Acceleration (g)",
                          file_name=ground_motion_file_name)
        # Assuming analyze_sdof_response parameters are ready
        analyze_sdof_response(ground_motion_acceleration, dt, k, c, m, j, folder_path)
...
up_bound_data = np.load('outputs/ls_upper_bounds_all.npy', allow_pickle=True)[0]
up_bound_data_2 = np.array(up_bound_data, dtype=np.float64)
low_bound_data = np.load('outputs/ls_lower_bounds_all.npy', allow_pickle=True)[0]
low_bound_data_2 = np.array(low_bound_data, dtype=np.float64)
folder_path = 'outputs/result_sdof_26D_confi95'
...
try:
    ...
    fig.write_html(f"outputs/0706_sdof_26D/optimization_history.html")
